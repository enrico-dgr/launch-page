...
class Bot {
  ...
  private doCampaign = async (messageHandle: ElementHandle<Element>) => {
    ...
    await this.instateer.goToPage(parsedUrl);
    var instagramAction: string;
    var result: boolean | string = false;
    //
    const isAvailablePage = await this.instateer.isAvailablePage();
    if (isAvailablePage === false) {
      instagramAction = "Not available page";
      result = false;
    } else if (await containsText(messageHandle, like)) {
      instagramAction = "like";
      const isPrivateProfile = await this.instateer.isPrivateProfile();
